...
describe('docs-route-model — the section spine', () => {
  ...
  it('taxonomy snapshot: group ids + ui member counts are deliberate (r4)', () => {
    ...
    // Re-frozen 2026-09-05 (ontology R2): the figure + reference line
    ...
    // Re-frozen 2026-09-06 (icon-component-pipeline): the Icon
    // component lands in general (10->11) — 100 ui items.
    // Re-frozen 2026-09-07 (markdown-streaming): the streaming
    // markdown face joins data-display (16->17) — 101 ui items.
...
describe('docs-route-model — install targets & the legacy map', () => {
  ...
  it('svelte.config entries are the EXACT derived set (no silent prerender drift)', () => {
    ...
    const expected = new Set([
      // /probe-folder-css + /probe-popover-area + /parity.html are
      ...
      '/', '/probe-folder-css', '/probe-popover-area', '/parity.html', '/prototypes/list-item-fusion', '/docs.html', '/docs/components.html',
  ...
  it('built dist closes the loop for every canonical page (r2 P1-3)', () => {
    ...
    for (const extra of ['/docs/components/form.html', '/docs/registry.html', '/docs/recipes.html', '/docs/jx-pure.html', '/docs/variant-grammar.html', '/docs/llms-txt.html', '/docs/icons.html']) {
