...
describe('the dialog ghost scope (r14-2 → r14-9)', () => {
  it('raw footer content and CardFooter buttons default to ghost via Context; explicit wins', () => {
    ...
    // the CardFooter's grouped button — inheritance flows through the
    ...
    // r14-9 integration (the kernel dialect): the footer snippet renders
    // RAW in the band — CardFooter's grid rides directly under it (the
    // variant scope renders no element), and exactly one grouped cluster
    // exists
    expect(target.querySelector('[data-jx-card-foot] > .jx-card-foot-grid')).not.toBeNull();
    expect(target.querySelectorAll('[data-jx-card-foot] [data-jx-btngroup]').length).toBe(1);
    ...
    const group = target.querySelector('[data-jx-card-foot] [data-jx-btngroup]')!;
