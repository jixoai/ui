...
<div class="mx-auto flex w-full max-w-[90rem] flex-col gap-8 px-4 pb-10 sm:px-6 lg:px-8">
  ...
  <div id="stacking" data-reveal=""><SectionCard family="stacking" headerRegion="stacking" eyebrow="toast-v2" title="The stack: depth, swipe, the hidden page" summary="One grid cell holds every wrapper (grid-area 1/1) — the stack DIALECT, not overlaid positions. THE GROWTH LAW (sonner's): the pile grows AWAY from its anchor edge — the adopted float plane anchors top-right so the pile descends into the content; the standalone fallback anchors bottom-right and climbs. Collapsed, each card sits one gap deeper and 5% smaller per depth; hover or touch expands to the full list while back boards keep the geometry and surrender only the ink (text-transparent — subtraction, never reflow). Swipe dismisses toward the stack's nearest screen edges; a hidden page freezes every clock — three orthogonal freeze sources, one timer; an expandable toast morphs into its reading posture through a view transition."><div class="grid gap-4 sm:grid-cols-2">
    ...
    <div class="border border-border p-4 grid gap-2"><PressButton onclick={() => toast.api.push({ title: 'Draggable', description: 'drag me toward the right screen edge — or up' })}>swipe to dismiss</PressButton><p class="text-sm text-muted-foreground">A carry at or past 48px — or a flick above 0.11 px/ms — dismisses along an allowed axis; the cross axis carries at 0.2 friction. Defaults come from the posture (top-right → right + up, toward the nearest edges); a push names its own.</p></div>
