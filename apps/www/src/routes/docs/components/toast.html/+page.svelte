  ...
  <div data-reveal="">
    ...
    >
      <div class="flex flex-wrap gap-3">
        ...
        <PressButton
          onclick={() => toast.api.push({ title: 'Build failed', variant: 'tonal', class: '[--jx-tonal:var(--error)]', assertive: true, duration: 0 })}>
          sticky · assertive
        </PressButton>
        <PressButton
          onclick={() => {
...
<div class="mx-auto flex w-full max-w-[90rem] flex-col gap-8 px-4 pb-10 sm:px-6 lg:px-8">
  <div id="types" data-reveal=""><SectionCard family="types" headerRegion="types" eyebrow="types" title="Toast variants" summary="The ladder drives border + ink over the floating popover ground; hue is injected through the class seam (a failed status injects --error — never the destructive action hue); assertive switches the live-region role; duration 0 is sticky.">
    <div class="grid gap-4 sm:grid-cols-3">
      ...
      <div class="border border-border p-4"><PressButton onclick={() => toast.api.push({ title: 'Build failed', description: 'error status · assertive · sticky', variant: 'tonal', class: '[--jx-tonal:var(--error)]', assertive: true, duration: 0 })}>error</PressButton></div>
    </div>
  </SectionCard></div>
