...
<ToastViewport store={toast} />`;
  ...
  const universalUsage = `<ToastViewport store={store} elevation="level4" />`;
...
<div class={cx(rt.shellFlush, rt.flex, rt.col, rt.gap32)}>
  ...
  <div id="universal-props" data-reveal="">
    <SectionCard
      ...
      summary="The eight-axis surface (explicit-props): size · shape · radius · density · color · theme · elevation · motion — each axis takes named steps, auto (inherit the ambient context; stamps nothing), an exact number (px · coefficient · dp · hue per axis), or query() for responsive/container-conditional values. The VIEWPORT owns the surface: the eight axes resolve in its mount context and the carriers stamp the STACK root — the batch C portal law in its purest form (the stack adopts into the scaffold's float plane through a REAL portal; a trigger ancestor's CSS carriers never span the adoption). The toast's own elevation level3 (6dp, M3's snackbar rung) feeds the card's rest shadow and the neutral ground's ladder rung."
    >
      <ComponentCanvas title="Toast · universal props" stage="fill" files={universalFiles}>
<div class={cx(rt.panel)}><p class={cx(rt.text13)}>The viewport mounts once; push toasts at the store — every card's rest shadow composes the level table's recipe and the neutral material's ground steps the paired rung.</p></div>
      </ComponentCanvas>
    </SectionCard>
  </div>
