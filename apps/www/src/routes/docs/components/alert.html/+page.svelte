...
<Alert variant="tonal" class="[--jx-tonal:var(--error)]" assertive={true} title="Build failed">
      ...
      {#snippet playground()}
        <PlayFields>
          <PlayHelp>
            ...
            the error injection (<code class="[--jx-tonal:var(--error)]">[--jx-tonal:var(--error)]</code>)
...
<div class="mx-auto flex w-full max-w-[90rem] flex-col gap-8 px-4 pb-10 sm:px-6 lg:px-8">
  <div id="types" data-reveal=""><SectionCard family="types" headerRegion="types" eyebrow="types" title="Alert variants" summary="Two ladder variants and two live-region roles; title and body are each optional. Hue is injected, never a variant name.">
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <div class="border border-border p-4"><Alert variant="tonal" class="[--jx-tonal:var(--error)]" assertive={true} title="error status — assertive">role=alert, immediate announcement; failed statuses inject --error — destructive is an action hue, not a status.</Alert></div>
