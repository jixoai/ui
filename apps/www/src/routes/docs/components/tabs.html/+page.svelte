<script lang="ts">
  ...
  import TabsList, { blurSlide, progressBlur, shadow } from '$lib/ui/tabs/tabs-list.svelte';
...
<Tabs bind:value>
  ...
  // The opacity wash densifies as the trigger NARROWS and stays capped
  // low enough for the label's foreground ink to read on top (V1-5)
  ...
  <div id="layouts" data-reveal="">
    ...
    >
      <div class="flex w-full max-w-2xl flex-col gap-7">
        ...
        <div class="flex flex-col gap-2.5">
          <span class="font-nav text-primary text-[11px] uppercase tracking-[0.24em]">scroll · scrollEffect=blurSlide()</span>
          <Tabs value="gamma">
            <TabsList layout="scroll" scrollEffect={blurSlide()} class="max-w-md">
              <TabsTrigger value="alpha">alpha</TabsTrigger>
              <TabsTrigger value="beta">beta</TabsTrigger>
              <TabsTrigger value="gamma">gamma</TabsTrigger>
              <TabsTrigger value="delta">delta</TabsTrigger>
          ...
          <span class="text-muted-foreground text-[12px]">triggers blur, fade and slide as they clip under the run's edges — each ramp follows its own clipped fraction (scroll-following stamps, every engine); radius and distance are builder options.</span>
        </div>
        <div class="flex flex-col gap-2.5">
