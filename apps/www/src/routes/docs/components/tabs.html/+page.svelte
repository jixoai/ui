...
>
  ...
  <div data-reveal="">
    ...
    >
      <div class="flex flex-wrap gap-3">
        ...
        <span class="pill">inline · grow · scroll</span>
      </div>
    </SectionCard>
  </div>
  ...
  <div id="layouts" data-reveal="">
    <SectionCard
      ...
      eyebrow="layouts"
      title="Layouts — inline, grow, scroll"
      summary="The strip's geometry is a prop, and it composes with every material. inline (the default) sizes the strip to its content. grow stretches every trigger to an equal share of the full width — the Material full-bleed top bar. scroll turns the strip into a horizontal overflow run with the scrollbar hidden: wheel, trackpad and the arrow walk keep traveling. Grow and scroll accept any indicator — the demos stage grow with pill and scroll with the default line on purpose."
    >
      <div class="flex w-full max-w-2xl flex-col gap-7">
        ...
        <div class="flex flex-col gap-2.5">
          ...
          <span class="text-muted-foreground text-[12px]">twelve triggers in the overflow run — the scrollbar is hidden, the walk is not.</span>
        </div>
      </div>
    </SectionCard>
  </div>
