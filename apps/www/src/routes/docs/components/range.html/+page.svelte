...
<!-- decimal steps snap at the step's precision; ticks = one mark per step,
     inset to the thumb's travel — click a mark to snap, the wheel
     fine-tunes one step per notch -->
      ...
      {#snippet playground()}
        <PlayFields>
          ...
          <PlayHelp>
            ...
            ruler insets to the thumb's travel: click a mark to snap, wheel = one step per
            notch (trackpad deltas accumulate, pinch-zoom is never hijacked).
          </PlayHelp>
        </PlayFields>
      {/snippet}
    </ComponentCanvas>
...
<div class="mx-auto flex w-full max-w-[90rem] flex-col gap-8 px-4 pb-10 sm:px-6 lg:px-8">
  <div id="types" data-reveal=""><SectionCard family="types" headerRegion="types" eyebrow="types" title="Range variants" summary="Use a plain slider for continuous input, add steps and ticks when values are discrete, or flip the axis — vertical rides the platform's own vertical face with min at the physical bottom."><div class="grid gap-4 sm:grid-cols-3"><div class="border border-border p-4"><Range label="continuous" value={40} /></div><div class="border border-border p-4"><Range label="stepped" value={4} min={0} max={10} step={1} /></div><div class="border border-border p-4"><Range label="with ticks" value={50} ticks /></div><div class="border border-border flex items-center justify-center p-4"><Range label="vertical fader" orientation="vertical" value={30} ticks min={0} max={10} step={1} /></div></div></SectionCard></div>
