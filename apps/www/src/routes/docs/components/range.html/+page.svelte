...
<div dir="rtl"><Range label="volume (rtl)" bind:value={v} /></div>`;
  ...
  const universalUsage = `<Range label="px number" min={0} max={100} value={40} size={14} density="small" />
<Range label="named steps" min={0} max={100} value={60} size="large" radius="medium" />`;

  const universalFiles: TreeFile[] = [
    { name: 'src/lib/ui/range-universal.svelte', content: universalUsage },
  ];
