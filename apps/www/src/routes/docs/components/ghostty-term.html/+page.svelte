...
export default {
  ...
  const axisRows = [
    ...
    },
    {
      ...
      description:
        "THE SHADOWED SLOT (the §13 case this campaign named the ruling after): the family's theme prop is the ABSENT-SLOT escape hatch — a structured { background, foreground, selectionBackground, … } object; ABSENCE is the meaningful state (the token sheet owns every shell color). 'light'/'dark'/'system' are not lanes here. THE MEASURED SPLIT: the surface atom is a typed :root token; the canvas shell probes the RAW shell tokens live but unobserved — a scope flip moves neither (measured: under a scoped .dark the ground holds oklch(0.9551 0 0) and ink oklch(0 0 0) while the scope's own --terminal reads the dark pole; the CSS surface holds by alias-freeze, the canvas by no-observer — there is no MutationObserver on class/data-theme, so the cached raw value paints until a theme-prop change re-probes the then-current scope), and the CONTENT is the wasm's ANSI — absolute under any theme. A terminal is deliberately theme-independent: the surface typed-frozen by construction, the shell pole-holding by no-observation, ANSI-verbatim by contract. No number lane, no enum lane.",
    },
    {
      name: 'elevation',
