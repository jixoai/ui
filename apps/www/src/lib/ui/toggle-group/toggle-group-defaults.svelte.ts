 ...
 *   - density: the no-opinion axis slot. The native segment row carries
 *     NO density own (the root stamps its own tier and provides family
 *     STATE — ToggleGroupApi — on a separate key, never the density
 *     axis): no provider and no explicit prop resolve undefined, stamp
