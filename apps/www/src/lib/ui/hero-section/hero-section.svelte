...
<script lang="ts">
  ...
  import { HeroSectionDefaults } from './hero-section-defaults.svelte';
  import { heroStyles } from './hero-section.stylex';
  import './hero-section.css';

  interface Props extends Omit<HTMLAttributes<HTMLElement>, 'title' | 'color'> {
    eyebrow: string;
    summary: string;
  ...
  // no-own: the hero's cm-measure typography and entrance cascade are
  // its OWN laws; the supply chain is the point — the composed CTAs
  // inherit the lanes)
  const d = $derived(
    HeroSectionDefaults.resolve({ density, size, shape, radius, color, theme, elevation, motion }),
  );
