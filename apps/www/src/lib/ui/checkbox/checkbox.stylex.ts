// checkbox.stylex.ts — the checkbox family's atom table
// (tailwindless one-shot Wave 1b batch C, 2026-09-17).
//
// Source of record: the ONE utility pair the markup carried — the
// bare posture's inline host (inline-flex w-fit: with no label/error
// to stack, the field wrapper is dead weight; inside list-item end
// lanes the control must sit at inline-END, not stretch the lane).
// Everything else is hook classes — .jx-field/.jx-label/.jx-error
// consumed from the jx-pure sheet, .jx-check-lane / .jx-html-checkbox
// from checkbox.css — all kept verbatim (the paint law is css-owned).
//
// Law mapping: display + width are structural — no theme slots.
//
// Mirror law: this file is byte-identical in
// registry/files/ui/checkbox/ and apps/www/src/lib/ui/checkbox/
