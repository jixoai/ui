<!--
  jixoai SystemDialog — the ROOT of the composition-first family
  (registry/files/ui/system-dialog/system-dialog.svelte, 2026-08-25).
  ...
  through context (shadcn-shaped composition:
    ...
    <SystemDialog bind:open onconfirm={del}>       ← state context only
      <SystemDialogTrigger>delete repo</SystemDialogTrigger>
      <SystemDialogContent>                        ← popover="manual",
        <SystemDialogTitle>delete repo?</SystemDialogTitle>       anchored
        <SystemDialogDescription>no undo</SystemDialogDescription>  to the
        <p>…free body…</p>                                    trigger, no
        <SystemDialogActions>                                  light
          <SystemDialogCancel>cancel</SystemDialogCancel>       dismiss
...
<script lang="ts" module>
  ...
  export const SYSTEM_DIALOG_KEY = Symbol.for('jx-system-dialog');
</script>
...
<script lang="ts">
  ...
  interface Props {
    ...
    open?: boolean;
    /** the agreement seam: runs on SystemDialogAction, then the dialog
  ...
  setContext<SystemDialogApi>(SYSTEM_DIALOG_KEY, {
    uid: autoId,
    get open() {
      return open;
    },
