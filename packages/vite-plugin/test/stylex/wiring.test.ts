...
// `components`; PFINAL W4-r2: the utilities tier DIED with the engine —
// the canonical prelude ends at components(.stylex.*))
const F9_CANONICAL_N3 =
  '@layer properties, theme, base, components, components.stylex.priority1, components.stylex.priority2, components.stylex.priority3;';
const F9_CANONICAL_SHEET = '@layer properties, theme, base, components;';
...
describe('the stylex feature (build side, F11)', () => {
  it('the semantic canonical matcher is whitespace/minification tolerant (Gate-2 r4)', () => {
    ...
    // PFINAL (W4-r2): the engine's internal prelude is now
    // BYTE-IDENTICAL to the canonical sheet statement (the utilities
    // tail that distinguished them died with the engine) — the
    // tolerant matcher strips every variant; removing the duplicate
    // and re-prepending the single canonical statement is
    // registration-inert (identical names, same order)
    ...
    expect(countCanonicalStatements('@layer theme, properties, base, components;')).toBe(0);
    expect(countCanonicalStatements('@layer properties, theme, base;')).toBe(0);
    expect(countCanonicalStatements('@layer utilities;')).toBe(0);
  });

  it('canonicalLayerStatement pins the F9 canonical statement verbatim (dynamic tiers; PFINAL — no utilities tier)', () => {
    expect(canonicalLayerStatement(3)).toBe(F9_CANONICAL_N3);
    expect(canonicalLayerStatement(0)).toBe(F9_CANONICAL_SHEET);
    // monotone in N: every tier listed exactly once; the utilities tier
    // is GONE (PFINAL) — the statement ends at the last tier
    expect(canonicalLayerStatement(9).endsWith(', components.stylex.priority9;')).toBe(true);
    expect(canonicalLayerStatement(9)).not.toContain('utilities');
