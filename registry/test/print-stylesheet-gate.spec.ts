...
describe('kernel-print.css — the AST gate', () => {
  ...
  it('carries the r5 furniture: running-head sources, whisper type + clip, paper code leading, the flattened card inset', () => {
    ...
    // rule text
    expect(kernelClean).toMatch(/h1\s*\{[^}]*string-set: docTitle content\(\)/);
    expect(kernelClean).toMatch(/h2\s*\{[^}]*string-set: sectionTitle content\(\)/);
    // the margin furniture whispers (the nav face, small, muted) and
    // an overlong head clips to ellipsis instead of crossing the top
    // edge — the corners only, the centered folio stays free
    expect(kernelClean).toMatch(/\.pagedjs_margin-content\s*\{[^}]*font-family: var\(--font-nav/);
