<!--
  jixoai AlertDialogSystem host
  (registry/files/ui/alert-dialog/alert-dialog-system.svelte;
...
<script lang="ts">
  ...
  import AlertDialogActions from './alert-dialog-actions.svelte';
  ...
  $effect(() => {
    if (open && mode === 'prompt') {
      requestAnimationFrame(() => {
        document
          .querySelector(':popover-open [data-jx-adlg-prompt-input]')
          ?.focus();
      });
    }
  });
...
<AlertDialog bind:open>
  <AlertDialogContent pose="center" focusLanding={mode === 'prompt' ? 'none' : 'cancel'}>
    ...
    {#if mode === 'prompt'}
      ...
      >
        <Input
          ...
          {placeholder}
          data-jx-adlg-prompt-input
        />
      </div>
    {/if}
    <AlertDialogActions>
      {#if mode !== 'alert'}
        <AlertDialogCancel onclick={() => settle(cancelValue)}>{cancelLabel}</AlertDialogCancel>
      {/if}
      <AlertDialogAction class={actionClass} onclick={() => settle(confirmValue)}>
        {confirmLabel}
      </AlertDialogAction>
    </AlertDialogActions>
  </AlertDialogContent>
