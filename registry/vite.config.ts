...
const jixoaiPlugins = jixoai({
  icons: {
    ...
    library: {
      ...
      // studio's settings face rides the committed canonical artifact
      // through its build's $lib alias — its six glyphs pack here in
      // the shared canonical face (consumer-need growth lane; MUST
      // stay byte-equivalent with apps/www/vite.config.ts and
      // scripts/gen-icon-set.mjs).
      icons: {
        boxes: 'lucide:boxes',
