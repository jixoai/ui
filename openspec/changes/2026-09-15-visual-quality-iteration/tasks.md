...
- [x] 1.1 Implement the scope ladder in `contextIsDark` (nearest ancestor
  ...
  'data-theme']`) with cleanup; the REPARENT trigger is explicit —
  attribute observation cannot see a host changing parents, so the
  effect rebinds/re-walks on reparent (or the boundary is documented)
- [x] 1.2 jsdom ladder battery: scope beats root beats OS; no-scope → OS;
...
- [x] 1.3 Browser pixel probe: light stage + `emulateMedia dark` → light
...
- [x] 1.4 Docs: effects page shimmer/rainbow demos keep working in both
  stage themes on an OS-dark machine (screenshot receipts)

## W2 — the mermaid backdrop (subtraction ink)

- [x] 2.1 `backdrop` prop (default on) + the subtractive veil
  ...
  transparent
- [x] 2.2 Contrast probe per the fixed acceptance: WCAG ratio, labels
...
- [x] 2.4 Docs: mermaid section gains the backdrop demo + prop row

## W3 — the timeline drawn spine

- [x] 3.1 The spine layer: whole-list SVG as a `grid-area: 1/1` sibling in
...
- [x] 3.2 The presets: plain / dashed (dot-edge phase anchor,
...
- [x] 4.2 `scroll-area` rework: always hand-drawn, capsule thumb,
  ...
  Props snapshot (the COMPLETE allowlist, frozen; no `scrollbar`
  field, no mode-shaped field of any name — axis capabilities like
  orientation are NOT mode-shaped; no `ScrollbarVariant` export),
  (2) a source scan for `'native'`/`'overlay'` consumer sites, (3) a
  two-directional fixture planting a live `scrollbar` prop proving
...
- [x] 4.4 Probe battery: verdict correctness (content fits/overflows,
...
- [x] 5.1 Flip the four inverted `position-area` sites to spec semantics
...
- [x] 5.2 The eight enumerated css paths (press-button, mermaid,
  ...
  ledger records them AND the NEW items' css files
  (native-scroll-area, scroll-area-kit) as they land
- [x] 5.3 `buildId`/`generatorVersion`: bump IFF generator inputs
  ...
  receipts
- [x] 5.4 MIRROR/PAYLOAD CLOSURE for every registry item this change
