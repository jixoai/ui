...
- [ ] 2.2 Contrast probe per the fixed acceptance: WCAG ratio, labels
  ≥ 4.5:1 vs node fill, fill/border ≥ 3:1 vs adjacent veil ground,
  pinned Chromium 2× screenshot sampling on the dark-pinned demo; any
  pair below threshold fails; derived-palette lift (through its own
  tokens) only if measurement demands, recorded
...
- [ ] 3.1 The spine layer: whole-list SVG as a `grid-area: 1/1` sibling in
  ...
  none`, `isolation: isolate` on the list root), ResizeObserver +
  childList + density re-measure, axis/direction/interlaced/RTL
  coordinate transform; the geometry payload contract (node centers in
  list-root coordinates, metadata, path data, density scale) exported
  and documented for custom spine snippets
