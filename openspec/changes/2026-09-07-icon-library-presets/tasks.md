...
- [x] A1: `src/icons/library/presets/` — the IconPreset interface +
       registry; `library.presets` option (string shorthand + object
       form); disabled/unknown preset prefix → named error listing
       the enabled set
- [x] A2: material preset (`md:` → @material-symbols/svg-${weight},
       weight/style/fill config, default outlined/400/FILL0) + peer
       absent loud-fail
- [x] A3: phosphor (`ph:` → @phosphor-icons/core) + remix (`rx:` →
       remixicon) presets + peer absent loud-fail
- [x] A4: adapter-side preset resolution through the shared
...
- [x] B1: factor the genuinely shared helpers out of
       ...
       font.ts)
- [x] B2: `{ font, code }` resolution → {v, n:'fill', d} via a new
...
- [x] B3: `{ font, liga }` best-effort GSUB lookup; miss = named
       ...
       the woff2-roundtrip precedent) — DECLARED LIMITATION:
       opentype.js cannot AUTHOR GSUB tables, so the in-memory
       fixture carries codepoint glyphs only; the liga HIT path is
       covered by the REAL JetBrains Mono woff2 (declared devDep,
       genuine GSUB fraction ligatures), no faked coverage

## C. Docs + gates

- [x] C1: icons page plugin section — preset table (prefix/package/
