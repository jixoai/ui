...
2. **16 components redesigned to compound families** (design.md has
   every target API). Audit verdicts across all 75 registered items:
...
PROP-DRIVEN (structure locked behind data)   → redesigned here
  ...
  pagination        fully closed: computes AND renders every link
  toggle-group      buttons behind options[{value,label}]
HYBRID (mild)                                 → opened here
  alert-dialog / popconfirm  action rows string-locked
...
3. **Ecosystem alignment** (research round, design.md sources): the
   ...
   shadcn-vue's Stepper and Dice UI's Timeline are the canonical
   targets for steps/timeline: `StepperItem step/Trigger/Indicator/
   Title/Separator`, `TimelineItem/Dot/Connector/Content/Title/Time`,
   with one state prop (`v-model` step / `activeIndex`) and everything
   else in the tree. shadcn-svelte's Svelte 5 translation rules apply:
   namespace barrels, implicit children snippets, `child({props})`
   replacing asChild.

## Impact

- registry/files/ui/{16 components} + mirrors at apps/www/src/lib/ui
  ...
  itself — site chrome migrates with it.
...
baseline, `scripts/verify-hook-law.mjs`, dev-server walkthrough of the
16 demo pages. Execution per the orchestration law: 6 parallel
