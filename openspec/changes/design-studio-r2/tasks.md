...
- [x] T4 选择模型 + 画布选择器：studio selection 状态
      ...
      VC1e）
- [x] T5 ComponentTreeView：frame 印章树 → studio 树面板，与
      picker 共享 selection（VC1 后半）
- [x] T6 chat 注入：选中 chip + 消息携带 selection + Agent 端
      定位提示词段（VC2）
- [x] T7 元数据按需提取：meta-gen 内核服务化（api/meta/<item>，
      锚定 itemAliasBase，typescript 直接依赖）+ 注解区校验 +
      canvas-schema x-ui 词表扩展（icon/i18n，delta 见
      specs/canvas-schema）（VD2/VD2h + 未知键）
- [x] T8 属性面板：schema → 控件 → AST prop 字面量重写（CAS 写
...
- [~] T9 Layout 族 alpha → **移交独立标准工作流**（worktree
      ...
      后集成阶段补
- [x] T10 CLI 面：design save/open/promote/status/apply 子命令
- [x] T11 studio UI 集成：selection chip / 树面板 / 属性面板 /
      updates-available 徽标（spec：studio surfaces drift）
- [x] V 系列全验（VA/VP 真车辆闭环；VC/VD vision 走查双轨；P0/P2 修复带 pin；VL1 rebase 后真源码往返（design.md §8；VC1 脚本断言 + vision 双轨）
      + VZ vision 三轮目击制走查
- [x] T12 openspec 收尾（spec 定稿 + validate --strict；vision 走查限界 P1-1/P1-2 记档 problems）：spec 定稿（含两处实测修订——"variant
