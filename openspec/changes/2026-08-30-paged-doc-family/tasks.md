...
- [ ] 1.1 `lib/paged/doc.svelte`：flow/columns/page 注入（print 档写
      `<style>` @page 规则）、orphans/widows 全局投影、runningHeader
      可选（Chromium-only 注释）、engine 占位、section 注册 Context。
- [ ] 1.2 `lib/paged/section.svelte`（break props + heading-keeper）、
...
- [ ] 1.3 `lib/medium.svelte.ts`：三态纯派生 reducer + sim 戳订阅 +
      SSR 安全；`lib/paged/print-projection.css`（unlayered 白名单表 +
      @media not print 的 sim 投影副本 + parity 注释）。

## 2. markup 合同 + 探针 [P]

- [ ] 2.1 props-table 包装层输出 `data-jx-props-table-scroll`；
      vitest source guard（禁手写 print 解绑替代）。
- [ ] 2.2 `scripts/verify-print.mjs`（verify-press 同款 playwright-core
