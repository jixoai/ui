...
| D1-03 | D1-01 + PerformanceObserver paint entries | at the FCP entry's timestamp, read probe el computed bg (poll via rAF from navigationStart, snapshot at FCP) | computed bg at FCP == authored value (styled first paint); the measured gap (ms) recorded either way | screen | dev |
