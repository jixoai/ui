...
| D1-03 | D1-01 + PerformanceObserver (paint) + resource timing + head MutationObserver | style-ready timestamp vs FCP. style-ready is DEFINED: prod = max(responseEnd) over the page's render-blocking `<link rel=stylesheet>` entries (resource timing); dev = performance.now() at the MutationObserver insertion of `style[data-stylex]`. FCP = paint entry startTime | PASS ⟺ style_ready ≤ FCP_start. If style_ready > FCP_start: gap ≤ 100ms → SOFT-FAIL (counts as FAIL, flagged for Owner overrule via ledger); gap > 100ms → hard FAIL. FCP, resource timing, style-ready, and the probe's settled computed bg are ALL recorded regardless of outcome | screen | dev+prod |
...
| D1-11 | three emulations | reduced-motion: animationName→none; forced-colors: recorded parity with current Part C degradation (property list pinned in spike); print: print-whitelist probe passes | all three recorded PASS against their pinned property lists | rm/fc/print | prod |
