...
> research/amendment-ledger.md (F4 grounded the rows in real repo
> CSS; F6 bound the manifest to the validator). ROW KINDS: rows
> citing `file.css:start-end` anchors are REPO-DERIVED (their
> selector/anchor is machine-verified against the repo sheets);
> rows whose expectation is authored INSIDE the spike are SYNTHETIC
> (their anchor is the frozen rule text quoted verbatim in this
> row — D2-01/03/06/07/08/09/11/13).
...
| D2-12 forced-colors | `.jx-pure` + `input[type='checkbox']` (real Part C law copied: `@media (forced-colors: active)` jx-pure.css:2255-2269, checkbox selector-list block jx-pure.css:2256-2264) | `@media (forced-colors: active)` block | appearance / background-image | appearance: auto AND background-image: none — identical computed pair in the coexist build (parity with the TW-only control build; no stylex-caused delta) | O1 |
