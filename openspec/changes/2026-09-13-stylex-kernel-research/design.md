...
> 的内核和 stylex 做集成".
>
> Baselines: source census @ 3c9097e0; protocol freeze @ F5 (the
> gate-1 r5 commit; see amendment-ledger.md). All protocol changes
> after F5 are ledger entries, never silent edits.
>
...
| build time cold/warm p50/p95 (nearest-rank, n=5 each) | measured | p95 Δ ≤ +15% |
...
**D3 measurement protocol — the baseline UNIT is frozen (Gate-1 r5
fixes the r4 sample-math contradiction)**:
...
- Environment (this machine, frozen): node v24.21.0, npm 11.19.0
  ...
  recorded per run.
- DELTA semantics: package-count and bytes vectors are DELTAS over
  the fixture's COMMON base (its svelte/vite/@jixoai dependency
  closure is the same in all four configurations and is excluded —
  the vector counts only the STYLING-ENGINE-related closure: TW side
  = tailwindcss + @tailwindcss/vite; stylex side = the engine's own
  dependency closure).
- Install/bytes: 3 DEDICATED `npm ci` runs per configuration (from
  the committed lockfile, rm -rf node_modules between), SEPARATE from
