...
`ui-tailwindless`) · Program: W1 → W1b → W2 → W3 → W4 (the Owner's one-shot
ruling, 2026-09-17: delete the engine entirely, then prove the 1:1 — and let
quality leap where the engine had been hard-coding).
...
## 4. Fresh probe receipts (2026-09-19)

- **Pilot parity matrix — 86/86 GREEN** (`pilot-matrix-receipt.json`):
  viewports 375/768/1099/1100/1440 × screenshots + computed-style reads
  (grid tracks across the 1100px seam, eyebrow voice, hairline frame, body
  voice, no `[object Object]`), dark-scope parity, keyboard focus,
  forced-colors, print emulation — after ≡ before on every row.
- **Precedence placement law — 25/25 GREEN** (`precedence-receipt.json`):
  every `.tl-*` family in-layer under components in `:where()`, media seams
  intact, ZERO unlayered regressions, injected consumer utilities FLIP both
  probe values (11px→41px eyebrow, semantic→#070809 ctl), native :hover
  seam alive.
- **folder-css contract — 8/8 GREEN** (item 6 reworked this wave): the
  layer-law arm now proves **a stylex atom beats a `:where()` folder rule**
  (8px→32px padding; lane recolors gray→primary oklch) — the engine that
  replaced TW utilities, proving the same inversion law the old arm did.
...
## 7. Gate receipts (final chain)

- verify:tailwindless — **GREEN, pin 0/0/0** (§1)
- verify:mirror — **GREEN** (144 items · 735 file pairs, manifest regenerated)
- verify:laws — **GREEN** (css-laws slots fresh, mirrors identical)
- verify:stylex-payload — **GREEN** · verify:stylex-authoring — **GREEN**
- verify:standards — **GREEN**
- verify:shadcn-add — **24/24 clean-install cases GREEN** (run 5,
  2026-09-19)
- verify:folder-css — **8/8 GREEN**
- verify:docs · verify:meta · verify:icons · verify:spins — **GREEN**
- pilot 86/86 · precedence 25/25 (§4)
- www battery — **2,833 passed · 6 failed, ALL pre-existing on main**
  (blueprints ×2, docs-ambient ×1, docs-structure ×2, tree-view ×2 —
  the identical known set; every W4-causal red repaired this wave:
  toast ×1 via the canonical-cx unification of toast-viewport/-dialog,
  text-modifiers ×1 via the cn-retirement contract, payload-parity ×1
  via the flipped-delivery derivation, markdown-ssr/search-corpus ×2
  via explicit 30s timeouts on the heavyweight SSR integrations)
- www build — **GREEN, engine-free**
