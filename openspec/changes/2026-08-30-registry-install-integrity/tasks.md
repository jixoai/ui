...
- [x] 1.2 Homepage: delete the stale `reveal` catalog row; reword every
      homepage mention of a reveal *action* (Law-03 paragraph, hero
      quick-start) to the `data-reveal` + CSS law.
- [x] 1.3 Grep sweep: no other file (docs pages, cli, skills) still
      instructs `npx jixoai-ui add reveal` or imports `@jixoai/reveal`.

## 2. The dependency gate

- [x] 2.1 Author `scripts/verify-deps.mjs` — ownership-resolved, not
...
- [x] 2.2 Wire `verify:deps` into `package.json` and `verify-all.mjs`
      (order: after `verify:mirror`). [integrator]
- [x] 2.3 Self-test: the four fixtures above as its unit test.

## 2b. The clean-install proof harness

- [x] 2b.1 Make `scripts/verify-shadcn-add.mjs` data-driven: each case
      ...
      harness.
- [x] 2b.2 Add the hero-section case: resolves AND requests no
      `@jixoai/reveal` dependency; canonical entry imports and builds.
- [x] 2b.3 `verify:all` executes the cases. [integrator]

## 3. The domain rides the artifact

- [x] 3.1 `build-site.mjs`: write `public/CNAME` (`ui.jixoai.com`) in
      the prepare phase (after empty, before dist copy); keep the
      existing "assert index.html + r/registry.json" step, extended to
      assert CNAME exists.
- [x] 3.2 `deploy.yml`: add a `smoke` job after the publish job —
...
- [x] 4.1 `catalog.ts` exports a `featured` projection: an explicit
...
- [x] 4.2 The "N items" heading renders the registry TOTAL from the
...
- [x] 5.1 Fix the `--brand-hue` comment on BOTH mirror sides
