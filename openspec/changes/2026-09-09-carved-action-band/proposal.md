# Proposal: carved-action-band — 动作带雕刻法则的泛化（sheet / system-dialog / canvas dock）
...
- **sheet foot 带**：ghost/flat zone 里是一条自带 `py-3.5` 的散排
  ...
  与按钮之间隔着一层 padding。
- **system-dialog actions 条**：同款 `py-3.5` 散排行 + `border-t`，
...
- **T3 system-dialog actions**：出血包装（mt/gap 呼吸算术与
...
- **T5 收尾**：registry.json 依赖边（system-dialog/component-canvas
