...
## T130 — 1st-review batch: pattern-login · progressive-blur · prose · result (2026-09-24)

- **The legacy-page debt signature is now mechanical**: all four pages carry exactly the page-side cx overload (:68/:51/:114/:57) + family cx clones (pattern-login ×2, progressive-blur ×1, result ×1) — the ?? {} one-liner closes seven of the eight. The two NON-cx seats: prose.svelte :281 (a nullable string into a string param — a narrowing, not the joiner) and result.svelte :132/:138 (Record<Props['status'], string> fails the key constraint because the optional prop carries undefined — NonNullable<…> is the shape). Batch reading: one saved scheck grep answered four page gates + four family lanes.
- **progressive-blur's three headline claims ALL verified live**: the 8-layer ladder digit-exact (computed backdrop-filters 0.5→64px), resting clean (every layer opacity 0.00 at scrollTop 0), the scroll-reveal (1.00 after a 200px scroll), the pin (band viewport-top constant across the range), hit-through + aria-hidden + pointer-events none. The family lane is otherwise CLEAN — the scroll-run :232/:241 narrowing fix (22e0d1b3) held; no new info.
- **The instrument lesson of the batch: anchor on content, not containers.** Three failed passes hunting the PB demo scroller by computed overflow (a non-scrolling wrapper, then the wrong scroller); the receipt came from anchoring on the demo's own li ("entry-01") and walking up to its true overflow container. Same class as the list-item li-vs-.jx-item face: the element you query first is often not the element that paints.
- **"Ships no script at all" is the popover zero-script class**: the mechanism claim (hero "zero JS" — sticky pin + scroll-timeline reveal, verified) is true, but the API summary's unscoped absolute fails view-source (the props lane is JS). Filed LOW with marginalia's own popover remedy: scope the phrase.
- **Token-equality beats hue-guessing**: result's success glyph == --primary (brand voice, no green) verified by computed-color equality with the served token — at the brand-hue wall-clock vintage the literal digit would have lied. error==--destructive (black at this vintage) and neutrals likewise.
- **prose is the strongest legacy page of the batch**: sovereignty verified BOTH halves at the paint (relaxed-markdown p 16/28 inside a measure-18/leading-2 region vs a plain P 18/36), absence-is-state (ambient host stamps nothing), gradient fill-only (clip:text + transparent fill + strong solid), chrome orthogonality (P muted grey vs chip black), indent 27px=2em, serif-pending honest (--font-serif undefined). Its gaps are coverage notes only (no link-in-prose seat for B2, no prose-list seat for B8).
- Receipts: reports 130-review-{pattern-login,progressive-blur,prose,result}.md — pattern-login 0M/1m/0L/0N, progressive-blur 0M/0m/1L/1N, prose 0M/1m/0L/0N, result 0M/1m/0L/1N; all Tier 2; build rc=0 @ 2b06c0d5, docs rc=0, universal 110/110, port 5243 clean, siblings (5241 t129 / 5244) verified by pid and untouched.

## T134 — 2nd-review batch: chart · language-switcher (2026-09-25)

- **The consolidation's fixes verified at every layer**: chart's :48 narrowing + 5 token-vocab seats + cx ×5 and ls's stale bullet + cx ×2 all closed — page gates 0/0, family lanes 0/0 errors on the batch's ONE saved scheck. The per-seat token-source reword healed the timeline-class recurrence (byte-check: all four stale strings zero-hit; the surviving source census is union-valid).
- **Second-review economy**: both pages PASS with zero new findings — the rare clean batch. The landed-item verification (type layer + served bytes) plus her receipt-set re-derivation (chart 23/23 role=img, fallback table inset(50%)+absolute, 5:2 ratio, axes texts; ls anchors/aria-current, pair/menu split, disclosure semantics) took two probe scripts total.
- **Ratio invariants travel, literals don't**: chart's 5:2 frame measured 532×212.8 at 1440 and 602×240.8 at 700 — vellum's digits (944×377.6) were her viewport; the invariant that reproduces is the RATIO 2.5. Second-review receipts should assert the invariant form.
- **Fresh axes that extend rather than repeat**: chart's NaN sweep (0 degenerate-path corruption — the a11y story closes at both layers: nameless impossible, senseless still well-formed); ls's keyboard disclosure contract (Enter-open with trigger-held focus, Tab-into-panel to the first locale, Escape/outside close) — the half T127 taught but didn't drive.
...
## T139 — 2nd-review batch: scroll-run · select · terminal-header (2026-09-25)

- **The ledgered-findings 2nd review has a third mode**: not "landed" (verify the fix) and not "open" (adjudicate new), but "standing" — re-verify the ledgered findings STILL exist honestly (chips tabindex="-1" ×2 ✓, the two focusable-teaching seats ✓, the naked clause ✓, the Components-pill seats ✓ behind <em> markup), re-derive the receipt set, and note partial healing: scroll-run's MINOR-2 (6 type seats) was ALREADY closed by the post-T131 fleet sweeps (page 0 on the saved run) — the ledger entry can record that half resolved.
- **The fresh axis completed a state machine**: marginalia's four-state census sampled start-closed/end-closed/none; my mid-travel drive sampled the FOURTH ("open", progress 0.479 = exactly half of max 159) — verdict machines get their traveling state from a fractional-progress drive, not a rest-state read.
- **Focus-restoration laws need a FOCUS-HOLDING opener**: TH's Escape-returns-focus-to-burger law false-failed under an evaluate-click (programmatic .click() never focuses the burger — nothing to restore, activeElement fell to BODY). Real-input clicks (Playwright click) focus the opener first; then Escape → focus restored ✓. The same rule likely governs every focus-restore receipt.
- **selected-vs-active is its own fresh axis**: select's aria-selected stays on "node" while aria-activedescendant roves — the listbox's two-state model, verified as distinct elements. Also: aria-activedescendant lives on the FOCUSED element (the UL.jx-sel-list), not the trigger — read the owner, not the expected site.
- **RTL re-derives need the right anchor**: the dir=rtl select seat ships no pre-selected row, so selected-row-anchored border reads can't fire there; the highlight row is the instrument (her T131 receipt stands).
- **Drawer link censuses drift with the docs tree**: TH's drawer read 222 links @500px (three navs: 138 hidden/140/222) vs her 279 at T131 — the card-6164px class: quote at-read-time numbers with the attribution clause.
- **Environment crash class**: verify:docs ENOENT'd on a dist page mid-sibling-consolidation (their build was rewriting dist/docs/components); the immediate re-run passed rc=0. A crashed fleet lint during concurrent consolidations is re-run first, diagnosed second.
- Receipts: reports 139-review-{scroll-run,select,terminal-header}.md — all three 0M/0m/0L/0N new, Tier 2 CONFIRMED ×3; build rc=0 @ 14c8ecd2, docs rc=0, port 5243 clean, siblings untouched.

## T145 — FINALE WALKTHROUGH group C: dialog · system-dialog · text · tour (2026-09-22)

- **The concurrent-build race is real and the hold protocol works**: my fresh-build step rc=1'd (ERR_MODULE_NOT_FOUND on .svelte-kit/output mid-prerender) because four walkthrough agents raced one apps/www workspace. Held per the orchestrator's correction, served the single-writer build READ-ONLY on ALL-CLEAR (`vite preview --port 5243 --strictPort`, no build), and the whole sweep ran on it with zero staleness. Lesson: walkthrough tasks should never own a build step when siblings run concurrently — the ALL-CLEAR read-only serve is the correct default shape.
- **Black-image defense fleet-scaled**: 87 captures histogrammed before ANY judgment — 87 non-trivial (min luminance std 27.0, max 72.0, threshold 20), zero suspects. The sips→BMP→node pipeline batches cheaply over a whole filmstrip set; it should run as one script over the capture manifest, not per-image.
- **Second instance of the eyeball-vs-numbers scrim class**: dialog-open.png visually read "no dimming"; pixel-diff found 740 differing bytes in the clip and luminance mean 170 (open) vs 229 (closed) = the EXACT 0.32 ::backdrop dim. Downscaled Reads of low-contrast overlays lie; the numeric luminance read is now mandatory for any scrim/tint claim.
- **The named playground check is an interactive receipt, not a static one**: driving the WEIGHT select to "550 · font-[550]" flipped the base paragraph's computed font-weight 500 → 550 live — including the ARBITRARY font-[450]/font-[550] rungs. "The sheet now computes" is proven by a state change you cause, with the options list as the vocabulary receipt.
- **Capture-artifact vs layout break, the disambiguator**: the text base paragraph "looked clipped under the dock" — measured scrollWidth 830 = clientWidth 830, baseRight < stageRight, and the viewport-fixed dock (bottom 1976) is flow-separated above the paragraph (top 2323). A fixed overlay at my scroll offset is not a break; overflowX/scroll-vs-client geometry settles it in one evaluate.
- **Mobile table edge-cuts get the overflow check**: system-dialog's API header "DESCI…" cut at 420 was wrapper overflow-x auto (scrollW 422 > clientW 322) — the DESIGNED horizontal scroll. One computed-style read separates the affordance from clipping.
- **Open-state captures need settle waits**: tour-open's first capture was byte-identical to the closed page (my click raced the reveal); diagnostic re-drive + settle waits got the anchored card. Also: shell-body filmstrip harness reused unchanged — it is the standing honest full-page instrument for the scaffold.
- Receipts: report 145-walkthrough.md — ALL FOUR PASS, Tier 2 standing, 0M/0m/0L/0N new; read-only on the single-writer build @ 48053c4f (verify:docs rc=0 orchestrator-verified, standing), NO svelte-check (nothing type-adjacent claimed), teardown by PID with lsof rc=1 empty, siblings untouched.

## T147-R1 — 1st review toggle (2026-09-24)

- **The tenth-seat class: code strings evade copy truing.** marginalia trued nine prose seats to the fusion V2, but the `usage` SNIPPET (:38) still opens "label reads on the LEFT of the control" — served ×2 (Usage CodeBlock + canvas drawer copy). Code-comment strings inside demo snippets are invisible to seat-by-seat prose passes; the retired-phrase census over SERVED BYTES catches them mechanically. Every copy-truing round should end with a byte census of the retired phrases, not a seat checklist.
- **Absence-is-state bit the rung probe**: `input[data-density="default"]` found NOTHING because the default rung stamps NO data-density — the stamp's absence IS the default state (and became a receipt in its own right). Census instruments keyed on stamped attributes must treat "missing" as a value.
- **Token-equality closed the knob-flip receipt**: knob checked bg oklch(0 0 0) == computed --primary-foreground oklch(0% 0 0) — at this brand the "foreground" is black, so a hue-intuition check ("knob should flip to white on primary") would have filed a false finding. Same lesson as result's success glyph.
- **aria-labelledby closes the label census**: the 19th toggle (playground PlayToggle) has no label[for] but IS named via aria-labelledby="jx-play-row-s8-label" — naming censuses need the labelledby clause or every playground row false-positives.
- **public/r payload layering pinned**: vite `npm run build` never touches public/r; the regenerator is build-site.mjs step 5/8 (`shadcn build` emits public/r/*.json at deploy), and verify-shadcn-add-universal rebuilds its own payloads. A stale tracked public/r between rounds is expected, not a defect — but "regenerates with the build" needs the deploy-layer qualifier in any gate record.
- **The two adjudications**: demo_tg ×3 ruled acceptable (no form ancestor — my submit receipt printed only "beta: yes"; names-not-ids), playground 'md' label filed NIT (values legal, generated code prints the truth; DensityDemo shows the proper xs/sm/default/lg vocabulary three sections below).
- Receipts: report 147-review-toggle.md — PASS 0M/0m/1L/1N, Tier 1 CONFIRMED; rungs xs 32×16 / sm 36×18 (driven) / default 40×20 / lg 48×24 digit-exact, slide matrix 0→20px live, knob+curve token/digit receipts, toc 9/9, LAW #19 82/0, retired-copy byte census clean but LEFT ×2; build rc=0 (dist 06:58), verify:docs rc=0, page 200, no svelte-check (dispatch), port 5243 clean (lsof rc=1).
