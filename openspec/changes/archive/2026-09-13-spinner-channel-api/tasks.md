...
- [x] 1. `src/spinners/channel/` module: `types.ts` (SpinnerChannel +
...
- [x] 2. `types.ts` config surface: `SpinnersPluginOptions.channels?`
      (additive); document the merge law in the option's doc comment.
- [x] 3. `resolve.ts`: `SPINNER_FULL_NAME_PATTERN` (flat ∪
...
- [x] 4. Pack factories: `magecdn({ pick? })` in
...
- [x] 5. `src/spinners/index.ts` re-exports `defineSpinnerChannel`
...
- [x] 6. `test/spinners/resolve.test.ts`: factory grammar errors
...
- [x] 7. `test/spinners/packs.test.ts`: `magecdn()` / `sam()`
...
- [x] 8. `test/spinners/generate.test.ts`: a namespaced artifact
...
- [x] 9. `test/spinners/vite-plugin.test.ts` + `umbrella.test.ts`:
...
- [x] 10. The THREE-config CONFIG-PARITY law: `apps/www/
