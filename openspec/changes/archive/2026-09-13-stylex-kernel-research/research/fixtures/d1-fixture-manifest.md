...
| D1-06 | dev AND prod loads with hydration; spike config pins `debug: true` in BOTH modes (data-style-src is debug-only — L1 §6) | capture console | ZERO messages matching /hydrat\|mismatch/i ∧ `data-style-src` attrs present after hydrate (debug-gated attribute; the non-debug prod assertion is dropped, not assumed) | screen | dev+prod |
