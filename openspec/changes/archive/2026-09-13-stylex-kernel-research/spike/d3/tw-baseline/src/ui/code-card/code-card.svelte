...
<script lang="ts">
  ...
  import type { HighlightBackend } from '../../lib/highlight/backend.ts';
  import {
    AUTO_LANG,
    type LanguageDetector,
    type DetectResult,
  } from '../../lib/highlight/lang-detector.ts';
  import {
    HIGHLIGHT_DETECT_KEY,
    type HighlightDetectContextValue,
  } from '../../lib/highlight/context-key.ts';
  ...
  import { HIGHLIGHT_KEY, type HighlightContextValue } from '../../lib/highlight/context-key.ts';
  // D3 fixture trim (disclosed in research/d3-consumer.md): the stock
  // shiki default backend is replaced by a plain-text null backend —
  // payload parity with the corpus code-card (NO highlight engine
  // ships in ANY D3 configuration; the backend seam itself stays).
  const DEFAULT_SHIKI_BACKEND: HighlightBackend = {
    id: 'd3-plain',
    async highlight() {},
  };
