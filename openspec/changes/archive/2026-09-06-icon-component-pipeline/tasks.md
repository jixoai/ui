...
- [x] P1: worktree bootstrap — `npm install` at repo root (lucide for
...
- [x] A1: `src/icons/library/` — options types (IconLibraryOptions,
...
- [x] A2: svgo integration — the FULL packaging chain (design §9):
       ...
       pack --dry-run` green
- [x] A3: structured extraction + packing — {v,n,d} extraction,
...
- [x] A4: `generateIconLibraryArtifacts()` pure core (design §5) —
       ...
       vite import)
- [x] A5: options wiring — IconsPluginOptions.provider optional,
...
- [x] A6: tests — packing boundaries (at/over budget, oversized
...
- [x] B3: gate migration — root package.json `gen:icons`/
...
- [x] B4: apps/www + registry vite configs both gain the library
...
- [x] C0: `scripts/verify-icon-migration.mjs` (--write/--check) +
       ...
       live hits)
- [x] C1: migrate `verify-shadcn-add.mjs` fixtures — exact-once
...
- [x] C2: migrate test consumers — jx-pure-parity.spec.ts,
...
- [x] C3: commit `scripts/icon-edge-ownership.json` (the 26-edge
...
- [x] C4: registry/files/ui/** (~33 files) + www chrome/docs/
...
- [x] C5: delete `registry/files/lib/icons.ts` + `apps/www/src/lib/
...
- [x] C6: living-spec migration (same change; openspec archive
...
- [x] D1: docs/icons.html rewrite — component API section,
...
- [x] D2: docs-structure snapshot + reading-chain coverage updated;
       icon + icon-set item pages/rows added

## E. Verification + closure

- [x] E1: `verify-all` green end-to-end on a clean worktree install
       (incl. mirror, deps, meta, laws, budgets, shadcn-add probes
       both tiers, verify:migration, the full test suite baseline)
- [x] E2: vision regression — icons docs page + theme-toggle + input
       semantic glyphs + tree-view + dialog, light/dark
- [x] E3: SSR + lazy-path spot proofs — server HTML contains core
...
- [x] E4: Codex review rounds on the real diff until sign-off;
