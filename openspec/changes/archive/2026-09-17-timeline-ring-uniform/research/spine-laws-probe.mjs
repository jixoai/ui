// The STANDING SPINE-LAWS probe (r6: the joint-lap + stroke-alignment
// laws merged into ONE probe; the ring dot's band depth now derives
// from the COMPUTED border width — no variant hardcoding, any token
// value proves). Supersedes the archived r4/r5 probes as the receipt.
//
// JOINT-LAP (r2, rebuilt after the r1 raster arm was caught
...
//      the same laws — its band is whatever its computed border says
//      (1px under the r6 uniform-weight ruling).
...
const MEASURE = () => {
  ...
  const dots = [...host.querySelectorAll('[data-jx-tl-dot]')].map((d) => {
    ...
    return {
      ...
      bw: parseFloat(getComputedStyle(d).borderTopWidth) || 0,
    };
  });
  ...
  for (const dot of dots) {
    // the hollow's INNER VERTEX RADIUS: a perpendicular border width
    // cuts √2× along the 45° bevel axis (r6: derived from the LIVE
    // computed border — no variant hardcode, any token value proves)
    const edgeIn = dot.r - dot.bw * Math.SQRT2;
...
const RASTER = async ({ b64, g }) => {
  ...
  for (let i = 0; i + 1 < dots.length; i++) {
    for (const [dot, nx, sign] of [[dots[i], dots[i + 1], 1], [dots[i + 1], dots[i], -1]]) {
      ...
      // the band's inner face sits at r − bw·√2 (the 45° bevel cut)
      const edgeIn = dot.r - dot.bw * Math.SQRT2;
  ...
  for (const dot of dots) {
    ...
    const edgeIn = dot.r - dot.bw * Math.SQRT2; // the hollow's inner vertex radius
...
try {
  ...
  // the r6 uniform-weight law: EVERY dot (ring included) borders at
  // the token, and the structural stroke rides the same weight
  const baseW = await page.evaluate(() => {
    const host = window.__probeHost;
    return { sw: parseFloat(getComputedStyle(host.querySelector('[data-jx-tl-base]')).strokeWidth) || 0,
             token: parseFloat(getComputedStyle(host).getPropertyValue('--jx-tl-stroke-w')) || 0 };
  });
  const bwSet = [...new Set(g.dots.map((d) => d.bw))];
  check('uniform weight: every dot border (ring included) === token === structural stroke', bwSet.length === 1 && bwSet[0] === baseW.token && baseW.sw === baseW.token,
    `borders [${bwSet.join(',')}], token ${baseW.token}, stroke ${baseW.sw}`);
  ...
  check('the ring variant rides the same DOM law (endpoint/mask, computed band)',
    ringLaw.ok && hasRing,
    ringLaw.fails.join('; ') || `axis=${rg.axis}, ${rg.dots.length} dots, mask r=${rg.maskR}, R=${rg.dots[0].r}`);
  const ringBw = rg.dots.find((d) => d.variant === 'ring')?.bw;
  check('the ring dot borders at the token (r6: no weight exemption)', ringBw === 1,
    `ring computed border ${ringBw}px`);
