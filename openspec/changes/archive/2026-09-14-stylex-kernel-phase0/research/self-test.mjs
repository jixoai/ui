...
const rows = [
  ...
  ['LAYER NEG reversed blocks', '@layer a{.x{color:red}}@layer b{.x{color:blue}}', '@layer b{.x{color:blue}}@layer a{.x{color:red}}', false],
  ['LAYER NEG name case', '@layer Foo{.x{color:red}}', '@layer foo{.x{color:red}}', false],
  ['LAYER NEG list order', '@layer a,b;', '@layer b,a;', false],
  ['LAYER pos statements vs list', '@layer a;@layer b;', '@layer a,b;', true],
  ['LAYER pos reopened same order', '@layer a{.x{color:red}}@layer a{.y{color:blue}}', '@layer a{.x{color:red}}@layer a{.y{color:blue}}', true],
  ['KF NEG name case', '@keyframes Spin{from{opacity:0}}', '@keyframes spin{from{opacity:0}}', false],
  ['KF NEG same-name override', '@keyframes spin{from{opacity:0}}@keyframes spin{to{opacity:1}}', '@keyframes spin{from{opacity:0}to{opacity:1}}', false],
  ['KF pos identical reopen', '@keyframes s{from{opacity:0}}@keyframes s{from{opacity:0}}', '@keyframes s{from{opacity:0}}', true],
  ['KF pos last-block wins', '@keyframes s{from{opacity:1}}@keyframes s{from{opacity:0}}', '@keyframes s{from{opacity:0}}', true],
  ['ANIM NEG ident case', '.x{animation-name:Spin}', '.x{animation-name:spin}', false],
  ['ANIM NEG shorthand ident', '.x{animation:3s Spin infinite}', '.x{animation:3s spin infinite}', false],
  ['VT NEG ident case', '.x{view-transition-name:Hero}', '.x{view-transition-name:hero}', false],
  ['MEDIA pos keyword case', '@media (MIN-WIDTH:12px){.x{color:red}}', '@media (min-width:12px){.x{color:red}}', true],
  ['FONT pos keyword case', '.x{font-family:ARIAL}', '.x{font-family:arial}', true],
  ['FONT NEG quoted case', '.x{font-family:"Arial"}', '.x{font-family:"arial"}', false],
  ['SUPPORTS NEG selector case', '@supports selector(.Foo){.x{color:red}}', '@supports selector(.foo){.x{color:red}}', false],
  ['SUPPORTS pos selector ws', '@supports selector( .Foo , .Bar ){.x{color:red}}', '@supports selector(.Foo,.Bar){.x{color:red}}', true],
  ['SUPPORTS pos keyword case', '@supports (DISPLAY:GRID){.x{color:red}}', '@supports (display:grid){.x{color:red}}', true],
