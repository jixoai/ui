...
- [x] A2: Face icon variable layering — --jx-icon-calendar/clock move
       from unlayered :root to @layer theme { :root { ... } } in
       jx-pure.css so the plugin's @layer theme vars can override them
       via import order
- [x] A3: Three-budget baselines — record B-source (jixoai+jx-pure
...
- [x] B4: Color-picker — VERIFIED: no input[type=color] in the composite
       ...
       input lives in input.svelte (already standard-sourced)
- [x] B5: Tags-input — internal input adopts jx-html-input
- [x] B6: Number-input — internal input adopts jx-html-input;
       steppers remain component-specific (platform stepper law)

## C. Plugin hardening (priority: medium)

- [x] C1: opentype.js externalization — configure tsdown to keep the
...
- [x] C2: HMR callback cleanup — the vite plugin's refresh path
...
- [x] C6: tsdown define warning — fix the invalid define config that
       produces a build warning (non-functional but noisy)

## D. Publishing readiness (priority: low, pre-release)

- [x] D1: package.json private → remove or set to false when ready
       to publish to npm
- [x] D2: Consumer documentation — README for @jixoai/ui-plugin with
       install + vite config + provider selection guide; document
       the virtual module import path
- [x] D3: Package-entry smoke test — verify that importing from the
       BUILT dist (not src) resolves correctly (types + runtime)

## E. Openspec hygiene (priority: immediate)

- [x] E1: Archive native-contract-fusion (move to
       openspec/changes/archive/)
- [x] E2: Archive ui-plugin-package (move to
       openspec/changes/archive/)
- [x] E3: verification.md for ui-plugin-package — record the r0-r5
