...
- [ ] 1. Spec deltas: css-architecture MODIFIED (isolation clause +
...
- [ ] 5. Enforcement: decide per the audit's gate survey (fold a
      z-index/isolation discipline check into an existing verify-*
      script, or carry the law in spec + review this round);
      implement if folded.
- [ ] 6. Vision verification on the preview (5218): the scroll-run
...
- [ ] 7. www test pass for touched components (scroll-run,
      component-canvas suites) + verify:all full gate.
- [ ] 8. Ship: verification.md, archive the change, commit, push
