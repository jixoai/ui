# collab-protocol Specification

## Purpose
The multi-role collaboration protocol behind design-studio, operationalizing
the five Owner rulings of 2026-09-15 (D1-D5): real-time concurrent editing
across the human panel, agent CLI writers, and out-of-band file edits over a
Loro motion-state kernel, an append-only adjudication journal, and the
`.svelte`/git static truth. Component identity is a native short `id`
(`a13`), edits are property-buffer ops with stable cursor anchors admitted
through a serialized gate, and conflicts are surfaced and arbitrated
(override / give-up) — never silently merged. The authoritative protocol
text lives in the archived change (`2026-09-16-collab-protocol/
protocol-spec.md`, v0.2.1); this spec is its living capability projection.
...
(RELEASE_SYNC, hash-pinned) with no module loader, dynamic-code and
prototype escapes reduced to non-configurable undefined,
deterministic Date/Math, controlled Promise, explicit disposal, and
memory/stack/interrupt budgets. Scripts SHALL have the
`group`/`parallel`/deps primitives; a group is one candidate fork,
one compile gate, one commit.
...
> `keep-partial` receipts per-op. The engine pin is
> `quickjs-emscripten@0.32.0` (WASM SHA-256 hash-gated). Scripts run
> once at admission and are archived as intent artifacts — only ops
> sync across devices (protocol-spec §7).
