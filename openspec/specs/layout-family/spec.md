# layout-family Specification

## Purpose
The alpha-track layout family (`prototype-flex` / `prototype-grid` / `prototype-waterfall`): three standardized layout primitives whose prop vocabulary the design studio's property panel edits live as CSS truth — zero translation, inline style only, zero dependencies, so the items render in ANY host that never wired the jixoai token sheet. Every member is single-root + rest-spread (the design-studio stamp precondition); the family is the stamp mechanism's in-repo self-proof, and the `meta.alpha` stamp carries no distribution exemption. Shared component laws ride component-authoring; this domain states only the different posture.
