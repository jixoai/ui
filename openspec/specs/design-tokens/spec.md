...
All colors SHALL be OKLCH with a single brand hue per project
...
The structure laws SHALL hold: hard offset shadows (no blur), radius 0
with `corner-shape: bevel` upgrade only (rounded-full reserved for
status-dot-class pills), mono-first typography (`--font-mono` body,
`--font-nav` nav), terminal surfaces (`--terminal*` tokens). Tailwind
v4 `@theme` mappings live in
...
`prefers-reduced-motion: reduce` SHALL neutralize reveal/motion
surfaces;
...
by written calc. Browser assertions SHALL compare USED values (raw
custom-property text is serialization-fragile: the `oklch(60% .2 25)`
vs `oklch(0.6 0.2 25)` lesson).

#### Scenario: a control alias drifts from its derivation

(the header keeps its historical name; the interface it SHALLs is
the post-rename `--jx-*` spelling below; the c31fe6a rename retired
the `--jx-d-*` / `--jx-d-ctl-*` spellings; component supplements live
under their component's own `--jx-<component>-*` names, e.g.
`--jx-toggle-track`)
...
is the post-rename `--jx-*` vocabulary) The verifier enforces this
closed set and rejects any OTHER ad-hoc density-token consumption in
family css as a second-scale attempt.
...
dedicated tokens. Tier semantics are physical, not decorative — the
tier definitions and named exemptions live in the note below the
scenarios.
...
> Geometry and range are expressed by mask gradients
...
tokens with their own tuning instead of re-declaring paint formulas.

#### Scenario: one tuning vocabulary, many paints

(component families tune by overriding the tokens: tabs 10px/1.5,
toast 12px/saturate 1, surface acrylic 14px/brightness 2/saturate 1.
The tokens' fallback values ARE the retired `.jx-glass` values
(14px / 1.35 / the 68% oklab color-mix of var(--background) /
brightness 1): computed-equivalent paint is preserved (an identity
function may append), and the two prior hard-coded spellings in the
theme sheet (`.jx-glass` utility — retired; `.jx-surface` acrylic
block) either flow through the family or through the tokens.)
